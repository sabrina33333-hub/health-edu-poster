...
**素材庫圖片是 16:9（1280x720），任何情境都不能用 `cover`（2026-09-08 踩過的坑，一開始以為只有滿版banner的問題，後來發現 `.split` 裡的一般圖片也一樣會被裁）：** 原本 `.art img` 用 `object-fit:cover` 撐滿容器，`.split` 裡的容器寬高比（約 430:75 甚至更窄）遠超過素材庫圖片實際的 16:9，`cover` 會把圖片上下裁掉大半（容器越窄越誇張），肉眼看縮圖不容易發現，是 Sabrina 實際看到「圖都會被裁切」才確認的。**現在 `.art img` 統一改用 `object-fit:contain`**（不分是否 `banner`），容器改用 `display:flex;align-items:center;justify-content:center` 讓圖片置中，寧可留白也不要裁到主體內容。`a4-handout.html` 已經是這個做法，套版時不要改回 `cover`。
