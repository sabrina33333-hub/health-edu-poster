...
- 2026-09-09：修正 `daily-selfcare-six-panel.png` 四周留白過大的問題——原始檔案（1280x720）實際內容只佔約 91% 面積，四周有明顯白邊，套版時這張圖用滿版展示（`.art.banner`），白邊被放大後在螢幕上看起來像「留白過多」（Sabrina 實測回報）。已裁切成 1190x631，內容緊貼邊界（只留 12px 呼吸空間），檔案已覆蓋原檔。**其餘 7 張角色插圖檢查過都是滿版無留白，只有這張是特例**（六宮格拼貼式的圖，跟其他單一場景滿版插圖的生成方式不同）。以後如果用類似「多格拼貼」方式生成新插圖，記得生成後檢查一下留白比例，不要預設一定是滿版。
